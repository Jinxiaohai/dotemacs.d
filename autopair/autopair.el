                     ...
                     (cl-destructuring-bind (innermost . outermost)
                       ...
                       (cond ((cl-first outermost)
                             ...
                             ((not (cl-first innermost))
                                                            (not (eq (autopair--pair-of (char-before (cl-third
                                                                                                      outermost)))
                                                                     autopair-inserted)))))))
...
`autopair-wrap-action'. "
    (when (and autopair-wrap-action
      ...
      (if autopair-handle-wrap-action-fns
          (condition-case err
            ...
            (scan-error (progn
                                                  (message "[autopair] error running custom
          `autopair-handle-wrap-action-fns', switching autopair off")
                                                  (autopair-mode -1))))
    ...
    (when (and autopair-action
      ...
      (if autopair-handle-action-fns
          (condition-case err
              (mapc #'(lambda (fn)
                                              (funcall fn (cl-first autopair-action) (cl-second autopair-action)
                                                       (cl-third autopair-action)))
                    autopair-handle-action-fns)
            (scan-error (progn
                                                  (message "[autopair] error running custom
               `autopair-handle-action-fns', switching autopair off")
                                                  (autopair-mode -1))))
